    ...
    <el-table :data="tableDatas" border style="width: 100%">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button v-show="!tableDatas[scope.$index].isEdit" size="mini" @click="Edit(scope.$index, scope.row)">编辑
          </el-button>
          <el-button v-show="tableDatas[scope.$index].isEdit" size="mini" @click="cancel(scope.$index,scope.row)">取消
          </el-button>
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        ...
        this.$http.post("http://localhost:8088/Shopping/api/v1/invoice/getinvoice", {
          "uid": this.uid
        }).then((res) => {
