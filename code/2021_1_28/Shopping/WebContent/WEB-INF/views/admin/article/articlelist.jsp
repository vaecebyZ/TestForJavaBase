...
<body class="blank">
	...
	<form id="forwordform"action='<c:url value="/admin/receiveUpdateArticle2?${_csrf.parameterName}=${_csrf.token}" />'method='POST'>
		...
		function userInfoListData() {
			...
			if (typeof oTable == 'undefined') {
				...
				oTable = $('#userInfoList').dataTable({
									...
									'bFilter' : false,
									"bSort" : false,
									'sAjaxSource' : contextPath+ '/admin/receiveGetArticle',
									"fnServerParams" : function(aoData) { // 查询条件
										debugger;
										aoData.push(
														{
															"name" : "title",
															"value" : $('[name=title]').val()
														},
														{
															"name" : "articleCategoryName",
															"value" : $('[name=articleCategoryName]').val()
														},
														{
															"name" : "updateTimeFrom",
