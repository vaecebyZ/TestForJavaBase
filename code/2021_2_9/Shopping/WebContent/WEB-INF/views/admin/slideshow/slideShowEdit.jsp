				...
				<div class="panel-body">
					<div class="row">
							<div class="hpanel">
								...
								<form id="form_slideShow" name="form_slideShow" action='<c:url value="/admin/save?${_csrf.parameterName}=${_csrf.token}" />'  method="POST">
									<input type="hidden" id="id" name="id" value="${slideshow.id}"/>	
									<div class="form-group col-lg-12">
										<label>幻灯片</label>
										<input type="text" id="name" name="name" class="form-control" maxlength="50" placeholder="幻灯片"	value='${slideshow.name}' />
										<input type="hidden" id="nameOld" value='${slideshow.name}'/>
									</div>
									<div class="form-group col-lg-12">
										<label>排序</label>
										<input type="text" id="sortOrder" name="sortOrder" class="form-control" maxlength="50" placeholder="排序" value='0' />
									</div>
									<div class="form-group col-lg-12">
										<label>图片</label>
										<input type="hidden" id="image" name="image" value='${slideshow.image}' />
                                           <!--dom结构部分-->
										<div id="uploader-demo">
										    ...
										    <div id="imageFile">
										    	<c:if test="${not empty slideshow.image}">
										    		<img src="${pageContext.request.contextPath}${slideshow.image}" style="width: 150px;height: 150px"/>
											    	</c:if>
											    	<c:if test="${empty pslideshow.image}">
											    		<img src=""/>
											    	</c:if>
										    </div>
										</div>
									...
									<div class="form-group col-lg-12">
										<label>描述</label>
										<textarea rows="3" cols="20" id="introduction" name="introduction" class="form-control" maxlength="2000"></textarea>
										<input type="hidden" id="introductionName" value='${slideshow.introduction}' />
									</div>
...
// 		console.log(ret.fileName);
	...
	$(function() {
		document.getElementById("introduction").innerText=$("#introductionName").val();
		$(document).ajaxSend(function(e, xhr, options) {
			xhr.setRequestHeader(header, token);
		});
		validate = $("#form_slideShow").validate({
			onsubmit:false,
			focusInvalid:false,
			errorClass:"error",
			onfocusout:false,
	...
	$("#back").bind("click",function(){
		window.location.href =contextPath+'/admin/slideshow';
	});
	
	/** 保存  */
	$("#save").bind("click",function(){
		if (!$("#form_slideShow").valid()) {
			return;
		}
		$.ajax({
			type : "POST",
			contentType : "application/json;charset=utf-8", 
			url : contextPath + '/admin/slideshow/save',
			data : formToJson($("#form_slideShow")),
			dataType : "json", 
			async : false,
